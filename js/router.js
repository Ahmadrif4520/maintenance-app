...
const loadPage = async (path) => {
    ...
    if (appContent) { // Pastikan appContent ada sebelum menggunakannya
        ...
        console.error("[Router] appContent element not found during loadPage.");
    ...
    if (user) {
        try {
            ...
            console.error("[Router] Error fetching user role:", error); // Debug log
    ...
    // Render halaman berdasarkan path
    switch (path) {
        case '/login':
        ...
        case '/dashboard':
            ...
            if (user && (userRole === 'admin' || userRole === 'technician')) {
                ...
                if (user) { // Jika user login tapi tidak punya akses, redirect ke login agar tidak stuck
                    navigateTo('/login');
                }
            }
            break;
        ...
        case '/machines/detail': // Contoh untuk detail mesin, perlu parameter ID
            if (user && (userRole === 'admin' || userRole === 'technician')) {
                ...
                if (machineId) {
                    ...
                    console.warn("[Router] Machine ID not found for /machines/detail."); // Debug log
...
auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
        ...
        if (notificationArea && setupNotificationListener) { // Pastikan elemen dan fungsi ada
